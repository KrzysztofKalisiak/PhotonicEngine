...
bool ph_sable_cell_occupied(ivec3 cell, ivec3 size, int atlas_z) {
    if (atlas_z < 0 || any(lessThan(cell, ivec3(0))) || any(greaterThanEqual(cell, size)))
        return false;

    return texelFetch(ph_sable_occupancy, ivec3(cell.xy, cell.z + atlas_z), 0).r > 0.5f;
}
...
bool ph_sable_matches_occupancy(vec3 grid_pos, ivec3 size, int atlas_z) {
    ...
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos)), size, atlas_z)) return true;
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos + vec3(probe, 0.0f, 0.0f))), size, atlas_z)) return true;
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos - vec3(probe, 0.0f, 0.0f))), size, atlas_z)) return true;
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos + vec3(0.0f, probe, 0.0f))), size, atlas_z)) return true;
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos - vec3(0.0f, probe, 0.0f))), size, atlas_z)) return true;
    if (ph_sable_cell_occupied(ivec3(floor(grid_pos + vec3(0.0f, 0.0f, probe))), size, atlas_z)) return true;
    return ph_sable_cell_occupied(ivec3(floor(grid_pos - vec3(0.0f, 0.0f, probe))), size, atlas_z);
}
