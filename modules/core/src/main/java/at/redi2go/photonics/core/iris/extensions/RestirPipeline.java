    ...
    public RestirPipeline(
        ...
        if (DISABLE_RESTIR_LIGHTING_PASSES_FOR_DIAGNOSTICS) {
            if (ENABLE_DIRECT_DIFFUSE_PASS_FOR_DIAGNOSTICS)
                Photonics.LOGGER.info("Photonics diagnostic: brute-force direct traced light sum with reverse visibility tracing and linear target-surface tolerance; Photon radiosity preserved; remaining ReSTIR passes disabled");
