    ...
    public RestirPipeline(
        ...
        if (DISABLE_RESTIR_LIGHTING_PASSES_FOR_DIAGNOSTICS) {
            if (ENABLE_DIRECT_DIFFUSE_PASS_FOR_DIAGNOSTICS)
                Photonics.LOGGER.info("Photonics diagnostic: nearest traced light first-hit mask (yellow selected light/end reached, green no voxel hit before light, red opaque blocker, cyan transparent blocker, blue outside selected light radius, magenta no traced lights); Photon radiosity preserved; remaining ReSTIR passes disabled");
