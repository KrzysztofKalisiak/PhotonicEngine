    ...
    public RestirPipeline(
        ...
        if (DISABLE_RESTIR_LIGHTING_PASSES_FOR_DIAGNOSTICS) {
            if (ENABLE_DIRECT_DIFFUSE_PASS_FOR_DIAGNOSTICS)
                Photonics.LOGGER.info("Photonics diagnostic: direct ray classification mask (red opaque blocker, blue transparent hit, yellow reached light, green no hit, black no candidate); Photon radiosity preserved; remaining ReSTIR passes disabled");
