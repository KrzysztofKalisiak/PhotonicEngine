...
public class RestirPipeline extends AbstractPhotonicsExtension {
    ...
    public RestirPipeline(
        ...
        Photonics.LOGGER.info(
                "Photonics ReSTIR configuration v57: directCandidatesPerPixel={}, directTemporalSampleCap={}, directOutputSampleCap=world-128/sable-temporal-cap, spatialCandidates={}, spatialRadiusPixels={}, outputDiagnostic=quarters-none/current-restir/pre-svgf-accumulated/sable-local, samplingPolicy=randomized-systematic-luminance-sorted-world-suffix+distinct-priority-prefix/sable-external-reservoir/exact-all-same-token-lights/fail-closed-tri-state-conservative-local-dda/preserved-zero-current-batches, spatialPolicy=receiver-matched-current-frame/external-only-sable/current-rejection-accounting/immutable-input/initial-batch-cap+rejected-fallback-cap+background-finalization, historyPolicy=receiver-domain-complete-split-stable-external/direct-only-full-rigid-local-history/camera-relative-double-compose/normal-guided-receiver/soft-local-signature-reset/visibility-transition-reset/representative-scoped-external-reactivity/explicit-emitter-domain/0.15-block-trail/2-frame-floor, motionHoldMs=250, denoiserPolicy=post-denoise-exact-local-hard-shadow+representative-gated-world+soft-local-signature-sable, sableSkyLightDiagnostic=transition-log/optional-freeze-getter, requestedDenoiserPasses={}, effectiveDenoiserPasses={}, softShadows={}, combinedGi={}",
        ...
        var currentLightingFramebuffer = irisFactory.newFramebuffer(properties.getRenderScale())
                .addAttachment("restir_current_lighting", ITextureFormat.rgba16f(), CREATE_SAMPLER, this::isBlockLightEnabled)
                .build(this::registerComponent);
        ...
        irisFactory.newPipeline()
                ...
                .withFramebuffer(currentLightingFramebuffer)
                .deferredPass("capture current lighting", "/photonics/rendering/restir/passes/r6_capture_current.fsh", null, this::isBlockLightEnabled)
