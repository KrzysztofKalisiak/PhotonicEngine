    ...
    public RestirPipeline(
        ...
        if (DISABLE_RESTIR_LIGHTING_PASSES_FOR_DIAGNOSTICS) {
            if (ENABLE_DIRECT_DIFFUSE_PASS_FOR_DIAGNOSTICS)
                Photonics.LOGGER.info("Photonics diagnostic: direct visibility mask (green visible, red blocked, black no direct candidate); distance-limited hard visibility and conservative opaque voxel thickness; Photon radiosity preserved; remaining ReSTIR passes disabled");
